...
function TelRow({ label, value, color, mono }: { label: string; value: string; color?: string; mono?: boolean }) {
  return (
    <div className="flex items-baseline justify-between">
      ...
      <span className={`text-[10px] ${mono ? "text-zinc-600" : ""} font-mono truncate max-w-[140px]`} style={color ? { color } : undefined}>
        {value}
      </span>
    </div>
  );
    ...
    <div className="flex flex-col border-b border-[#27272a]">
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b border-[#27272a]">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span className="text-[11px] font-bold text-zinc-200 truncate">{asset.callsign}</span>
          <span
        ...
        <button onClick={onClose} className="text-zinc-600 hover:text-zinc-300 transition-colors cursor-pointer shrink-0">
          <X size={13} />
        </button>
      </div>

      {/* 3D Model — overflow hidden + oversized iframe hides Sketchfab watermark */}
      {embedSrc && (
        <div className="w-full h-[150px] border-b border-[#27272a] overflow-hidden relative">
          <iframe
